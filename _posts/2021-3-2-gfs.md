...
A larger chunk size helps a lot in the system performance:
...
* Clients can hold and modify more data locally, reducing TCP connections with servers from frequently uploading requests. 
...
Separate data and control flow is a common practice for high performance. Clients ask the master for locations of file chunks, but data exchange is established directly with chunckservers. Also, the master is responsible for managing data, including balancing data distribution, reclaiming space from deleted or stale chunks, and providing serialized plans of concurrent mutations.
